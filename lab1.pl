...
solve(Prems, Goal, [Row | Tail], IsBox, PreviousBox1, PreviousBox2) :- 
	...
	(Row = [[RowNumber, Variable, assumption] | _] ,  
		(write('Box opened! '), PrevBox2 = PreviousBox1, PrevBox1 = Row,  solve(Prems, Goal, Row, 1, PrevBox1, PrevBox2),   NewPrems = Prems))) 
	-> solve(NewPrems, Goal, Tail, IsBox, PrevBox1, PrevBox2).

assert(Prems, Goal, [Row, P, assumption], _, _) :- write('Assumption! ').
...
assert(Prems, Goal, [Row, or(neg(X),  X), lem], _) :- write('LEM! ').
assert(Prems, Goal, [Row, or(X, neg(X)), lem], _) :- write('LEM! ').
...
assert(Prems, Goal, [Row, X, orel(A,B,C,D,E)], PreviousBox1, PreviousBox2) :-
	length(PreviousBox1, Len1), length(PreviousBox2, Len2),
	member([A, or(U, V), _], Prems),
	nth1(1, [B, U, _], PreviousBox1), nth1(Len1, [C, X, _], PreviousBox1),
	nth1(1, [D, V, _], PreviousBox2), nth1(Len2, [E, X, _], PreviousBox2),
	write('Or-El! ').
